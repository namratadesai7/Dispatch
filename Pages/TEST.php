...
if(!empty($billno)){
    $condition.="AND (  DDATE IS NULL   OR
    (DBILLNO = '$billno' AND DDATE >= CONVERT(DATE, CONCAT('01-04-', $startYear), 5) 
    ...
    <table class="table table-bordered text-center table-striped table-hover mb-0" id="distable">
                ...
                <tbody>
                    <?php
                        ...
                        $sr=1;
                        $sql="SELECT distinct(od.OrdID),o.customer,i.ProcessDrum,i.Size1,i.Length1,i.InsLength,i.DDRUMSIZE,od.JobNo as value ,i.DQTY,i.GrossWeight, i.SrNo
                        FROM OrdDetail od 
                        INNER JOIN OrdMaster o ON o.OrdId = od.OrdId
                        INNER JOIN Inspection i ON i.JobNo = od.JobNo
                        INNER JOIN Dispatch d ON d.JobNo=i.JobNo
